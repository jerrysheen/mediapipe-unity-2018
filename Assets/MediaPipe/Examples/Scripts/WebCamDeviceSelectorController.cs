...
public class WebCamDeviceSelectorController : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        Cam_Initialize(WebCamTexture.devices);
    }

    private void Cam_Initialize(WebCamDevice[] devices) {
        if (devices.Length != 0) {
            sceneDirector.GetComponent<SceneDirector>().ChangeWebCamDevice(devices[0]);
        }
    }
}
